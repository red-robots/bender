...
/*-----------------------------------------------------------------------------------*/
/*  bender Customizer Controls
/*-----------------------------------------------------------------------------------*/

class bender_Misc_Control extends WP_Customize_Control {
...
class bender_Theme_Support extends WP_Customize_Control {
	public function render_content() {
		//echo wp_kses_post( 'Upgrade to <a href="#">bender Plus</a> to be able to change the section order and styling!', 'bender' );
	}
}

if ( ! function_exists( 'bender_sanitize_checkbox' ) ) {
 ...
 */
function bender_sanitize_repeatable_data_field( $input , $setting ){
            ...
            if ( isset( $fields[ $id ] ) ){
                switch( strtolower( $fields[ $id ]['type'] ) ) {
                    ...
                    case 'color':
                        ...
                        break;
                    case 'checkbox':
                        $data[ $i ][ $id ] =  bender_sanitize_checkbox( $value );
                        break;
                    case 'select':
                        $data[ $i ][ $id ] = '';
 ...
 * @access public
 */
class bender_Customize_Repeatable_Control extends WP_Customize_Control {
    ...
    public function __construct( $manager, $id, $args = array() )
    {
        ...
        if ( ! isset( $args['allow_unlimited'] ) || $args['allow_unlimited'] != false ) {
            $this->max_item =  apply_filters( 'bender_reepeatable_max_item', absint( $args['max_item'] ) );
        }  else {
    ...
    public function render_content() {
        ...
        <div class="repeatable-actions">
            <span class="button-secondary add-new-repeat-item"><?php _e( 'Add a Item', 'bender' ); ?></span>
        </div>
            ...
            <div class="widget">
                <div class="widget-top">
                    ...
                    <div class="widget-title">
                        <h4 class="live-title"><?php _e( 'Item', 'bender' ); ?></h4>
                    </div>
                </div>
                                ...
                                <# if ( field.type ){ #>
                                    ...
                                    <div class="item item-{{{ field.type }}} item-{{{ field.id }}}">
                                        ...
                                        <# } else if ( field.type == 'media' ) { #>
                                            ...
                                            <div class="actions">
                                                <button class="button remove-button " <# if ( ! field.value.url ){ #> style="display:none"; <# } #> type="button"><?php _e( 'Remove', 'bender' ) ?></button>
                                                <button class="button upload-button" data-media="{{field.media}}" data-add-txt="<?php esc_attr_e( 'Add', 'bender' ); ?>" data-change-txt="<?php esc_attr_e( 'Change', 'bender' ); ?>" type="button"><# if ( ! field.value.url  ){ #> <?php _e( 'Add', 'bender' ); ?> <# } else { #> <?php _e( 'Change', 'bender' ); ?> <# } #> </button>
                            ...
                            <div class="widget-control-actions">
                                <div class="alignleft">
                                    <span class="remove-btn-wrapper">
                                        <a href="#" class="repeat-control-remove" title=""><?php _e( 'Remove', 'bender' ); ?></a> |
                                    </span>
                                    <a href="#" class="repeat-control-close"><?php _e( 'Close', 'bender' ); ?></a>
                                </div>
                                <br class="clear">
                            </div>
